...
// The same Debugger can be used concurrently from multiple [starlark.Thread]'s and goroutines,
...
// The same [Debugger] always returns the same builtin.
...
func (d *Debugger) makeBreakpoint() *starlark.Builtin {
	return starlark.NewBuiltin("breakpoint", func(thread *starlark.Thread, fn *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (val starlark.Value, err error) {
		...
		if _, loaded := threadsWithDebugSession.LoadOrStore(thread, nil); loaded {
			...
			printWithPos(thread, "breakpoint: reentrant breakpoint() call is ignored")
			return
		}
		...
		if !debugLock.TryLock() {
			printWithPos(thread, "breakpoint: concurrent debugging sessions attempted; blocking until other sessions finish")
			debugLock.Lock()
		}
		...
		printWithPos(thread, "breakpoint: Stopped")
		printWithPos(thread, "Available functions: c[ont[inue_]] (or Ctrl+D), q[uit], w[here]/b[ack]t[race], d[own], u[p], f[rame], vars, globals, locals")
...
// Unlike f.Globals(), starlark.Function doesn't provide a way to get the predeclared symbols.
...
func getPredeclared(f *starlark.Function) starlark.StringDict {
...
func printWithPos(thread *starlark.Thread, format string, a ...any) {
